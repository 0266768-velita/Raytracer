...
public class Triangle implements Object3D {
    ...
    // legacy constructor — takes raw Vector3D vertices and a color
    ...
    // legacy constructor — takes raw vertices and a material
    ...
    private static int embedT(Vector2D uv) {
        int i = tTop++;
        GUU[i] = (float) uv.getU(); GUV[i] = (float) uv.getV();
        return i;
    }
    ...
    // Möller–Trumbore intersection — all math done on primitives to avoid Vector3D allocations
    @Override
    public Intersection intersect(Ray ray) {
        ...
        double hy = rdz * e2x - rdx * e2z;
        double hz = rdx * e2y - rdy * e2x;

        double a = e1x * hx + e1y * hy + e1z * hz;
        if (a > -EPS && a < EPS) return null; // ray is parallel to the triangle
        ...
        if (t <= EPS) return null; // intersection is behind the ray origin
